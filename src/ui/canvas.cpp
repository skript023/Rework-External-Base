...
namespace ellohim
{
	...
	void canvas::check_for_input_impl()
	{
		...
		m_open_key_pressed = is_key_pressed(g_settings.window.open_key) || is_controller_pressed(XINPUT_GAMEPAD_BACK);
		m_back_key_pressed = is_key_pressed(g_settings.window.back_key) || is_controller_pressed(XINPUT_GAMEPAD_B);
		m_enter_key_pressed = is_key_pressed(g_settings.window.enter_key) || is_controller_pressed(XINPUT_GAMEPAD_A);
		m_up_key_pressed = is_key_pressed(g_settings.window.up_key) || is_controller_pressed(XINPUT_GAMEPAD_DPAD_UP);
		m_down_key_pressed = is_key_pressed(g_settings.window.down_key) || is_controller_pressed(XINPUT_GAMEPAD_DPAD_DOWN);
		m_left_key_pressed = is_key_pressed(g_settings.window.left_key) || is_controller_pressed(XINPUT_GAMEPAD_DPAD_LEFT);
		m_right_key_pressed = is_key_pressed(g_settings.window.right_key) || is_controller_pressed(XINPUT_GAMEPAD_DPAD_RIGHT);
		m_left_tab_pressed = is_key_pressed(g_settings.window.left_tab_key) || (is_controller_pressed(XINPUT_GAMEPAD_LEFT_SHOULDER));
		m_right_tab_pressed = is_key_pressed(g_settings.window.right_tab_key) || is_controller_pressed(XINPUT_GAMEPAD_RIGHT_SHOULDER);
	}
	...
	void canvas::draw_slider(float x, float y, float current_value, float min_value, float max_value)
	{
		...
		draw_centered_text(textbuf, text_center_x, text_center_y, text_color, renderer::get_monospace_font());
	}
	...
	void canvas::draw_footer()
	{
		...
		draw_left_text("Quantum", g_settings.window.m_pos.x + m_padding.x, m_draw_base_y + (m_footer_height / 2.f) + m_padding.y, Color(255, 255, 255, 255), renderer::get_font());
		draw_right_text("1.0", g_settings.window.m_pos.x + (g_settings.window.m_width - m_padding.x), m_draw_base_y + (m_footer_height / 2.f) + m_padding.y, Color(255, 255, 255, 255), renderer::get_font());
	...
	void canvas::draw_description()
	{
		...
		draw_left_text(
			...
			g_settings.window.description_text_color, renderer::get_font());
